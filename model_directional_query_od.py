...
# REPLACE EXISTING SSL CLASS WITH:
class SSL(nn.Module): 
    def __init__(self, channels):
        super(SSL, self).__init__()
        
        # PRE-INITIALIZE WAVELET TRANSFORMS (CRITICAL FIX)
        self.dwt = DWTForward(J=1, mode='zero', wave='db3')
        self.idwt = DWTInverse(wave='db3', mode='zero')
        
        # Keep existing convolutions
    ...
    def forward(self, x):
        ...
        # device = x.device
        # aa = DWTForward(J=1, mode='zero', wave='db3').to(device)
        
        # REPLACE WITH:
        yl, yh = self.dwt(x)
        
        # Keep rest of forward pass unchanged until line 46
        yh_out = yh[0]
        ylh = yh_out[:,:,0,:,:]
        yhl = yh_out[:,:,1,:,:]
        yhh = yh_out[:,:,2,:,:]
...
class Inpainting(nn.Module):
    ...
    def _forward_impl(self, x):
        # MOVE EXISTING FORWARD LOGIC HERE (lines 155-175)
