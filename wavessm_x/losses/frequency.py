...
class FrequencyAwareLoss(nn.Module):
    ...
    def __init__(self, wavelet='db3', levels=1, weights=None, loss_type='l1'):
        ...
        self.levels = levels
        self.weights = weights or {'LL': 1.0, 'HF': 1.5}
        self.loss_fn = nn.L1Loss() if loss_type == 'l1' else nn.MSELoss()

    def forward(self, pred, target):
        # ── FIX: keep loss computation inside float32 block ──
        # DWT output (especially HF subbands) can have large magnitudes
        # that lose precision when L1 runs in float16 outside this block
        with torch.amp.autocast('cuda', enabled=False):
            ...
            for i in range(self.levels):
                loss = loss + self.loss_fn(ph[i], th[i]) * self.weights.get('HF', 1.5)
...
class AdaptiveFrequencyLoss(FrequencyAwareLoss):
    ...
    def forward(self, pred, target):
        # ── FIX: entire computation in float32 ──
        with torch.amp.autocast('cuda', enabled=False):
            pl, ph = self.dwt(pred.float())
            tl, th = self.dwt(target.float())
