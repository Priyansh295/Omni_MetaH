...
instrum_pso = ng.p.Instrumentation(
    w_l1=ng.p.Scalar(lower=0.1, upper=0.8),
    w_percep=ng.p.Scalar(lower=0.2, upper=0.7),
    w_ssim=ng.p.Scalar(lower=0.25, upper=0.8),
    w_edge=ng.p.Scalar(lower=0.25, upper=0.8)
)
# DE for expansion factor
instrum_de = ng.p.Instrumentation(
    expansion_factor=ng.p.Scalar(lower=2.2, upper=3.0)
)
...
if __name__ == '__main__':
    ...
    if temp_args.optimize:
        # Parse all remaining arguments using the original parse_args function
        # Remove --optimize from sys.argv temporarily to avoid conflicts
        sys.argv = [sys.argv[0]] + remaining
        args = parse_args()
        
        # Store CLI arguments in variables for use in optimization
        data_path = args.data_path
        data_path_test = args.data_path_test
        save_path = args.save_path
        data_name = args.data_name
        num_iter_cli = args.num_iter
        
        print(f"Using CLI arguments:")
        print(f"  data_path: {data_path}")
        print(f"  data_path_test: {data_path_test}")
        print(f"  save_path: {save_path}")
        print(f"  data_name: {data_name}")
        print(f"  num_iter: {num_iter_cli}")
        
        # Run metaheuristic optimization with CLI arguments
        # GA
        ...
        plot_dir = os.path.join(save_path, 'hyperparam_plots')
        os.makedirs(plot_dir, exist_ok=True)
        # Plot all hyperparameters vs value
        ...
        # Use CLI num_iter for final training
        final_num_iter = num_iter_cli
        print(f"Starting final training with {final_num_iter} iterations")
